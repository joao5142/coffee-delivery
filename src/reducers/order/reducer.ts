...
export function orderReducer(state: OrderState, action: any) {
	switch (action.type) {
		case ActionTypes.ADD_NEW_COFFEE: {
			const coffeeIndex = state.coffees.findIndex((coffee) => coffee.id == action.payload.coffee.id);
		...
		case ActionTypes.REMOVE_COFFEE: {
			...
			return produce(state, (draft) => {
				draft.coffees = draft.coffees.filter((coffee) => coffee.id !== action.payload.id);
				setOrderToStorage(draft);
